...
def move_player(cur_heap: int, max_candies: int) -> int:
    ...
    if taken_candies > max_candies or taken_candies <= 0 or taken_candies > cur_heap:
        print(Fore.RED + 'Столько конфет взять нельзя. Переходите!')
        return move_player(cur_heap, max_candies)
...
if __name__ == "__main__":
    ...
    if number_game == 1:
        ...
        name2 = 'clever_bot'
    print(Fore.GREEN + f"Победитель: {play_candy(name1, name2, count_candies, max_candies)}")
